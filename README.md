...
(v) User Interface:
# Assumptions
Μια νοσηλεία χωρίς ΗΜΕΡΟΜΗΝΙΑ_ΕΞΟΔΟΥ θεωρείται «ανοικτή» — δεν έχει διάγνωση εξόδου και δεν κοστολογείται.  
Το ΣΥΝΟΛΙΚΟ_ΚΟΣΤΟΣ υπολογίζεται αυτόματα από trigger κατά το INSERT/UPDATE, βάσει ΚΕΝ + υπέρβασης ΜΔΝ. Δεν επιτρέπεται χειροκίνητη εισαγωγή τιμής.  
Κάθε κλίνη μπορεί να έχει μόνο μία ενεργή νοσηλεία ανά πάσα στιγμή.  
Κάθε άφιξη ασθενή δημιουργεί υποχρεωτικά μία εγγραφή διαλογής, ανεξάρτητα από το αν οδηγήσει σε νοσηλεία.  
Ο έλεγχος αλλεργιών γίνεται με LIKE (partial match) μεταξύ ονόματος δραστικής ουσίας και της καταγεγραμμένης αλλεργίας, λόγω ανομοιομορφίας στην ονοματολογία του αρχείου EMA.  
Συνταγογράφηση επιτρέπεται μόνο για ασθενείς με ενεργή νοσηλεία (χωρίς ημερομηνία εξόδου).  
Κάθε νοσηλεία αντιστοιχίζεται σε έναν ΚΕΝ κωδικό κατά την εισαγωγή. Το κόστος ανά ΚΕΝ διαφοροποιείται ανά ασφαλιστικό φορέα.  
Μόνο ένας ιατρός παρακολουθεί την εκάστοτε νοσηλεία (όχι ομάδα ιατρών)
Τα triggers εισάγονται μετά το load (install -> load -> triggers), εξασφαλίζουμε την ορθότητα/περιορισμούς στο ίδιο το py script, για την αποφυγή conflicts κατά την φόρτωση δεδομένων  
# Technical Details
και αυτα
